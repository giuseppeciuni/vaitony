...
class ProjectFile(models.Model):
    ...
    is_included_in_rag = models.BooleanField(default=True, help_text='Se True, il documento è incluso nella ricerca RAG')
...
# Signal aggiornato per gestire il toggle dei documenti
@receiver(post_save, sender=ProjectFile)
def update_project_index_on_file_change(sender, instance, created, **kwargs):
    """
    Aggiorna l'indice RAG quando un file viene modificato o quando cambia lo stato di inclusione.
    Implementa ottimizzazione intelligente degli embedding.
    """
    ...
    if created:
        logger.info(f"🆕 Creato nuovo file {instance.filename} per il progetto {instance.project.id}")
    else:
        logger.info(f"🔄 Aggiornato file {instance.filename} per il progetto {instance.project.id}")

    update_fields = kwargs.get('update_fields', [])
    is_rag_inclusion_change = update_fields and 'is_included_in_rag' in update_fields

    # Se è un cambio di inclusione RAG, usa ottimizzazione intelligente
    if is_rag_inclusion_change:
        if instance.is_included_in_rag:
            logger.info(f"✅ FILE {instance.filename} RIATTIVATO per la ricerca RAG - ottimizzazione indice")
        else:
            logger.info(f"❌ FILE {instance.filename} DISATTIVATO dalla ricerca RAG - ottimizzazione indice")

        try:
            # Usa la nuova funzione ottimizzata invece di force_rebuild=True
            create_project_rag_chain_optimized(
                instance.project,
                changed_file_id=instance.id,
                operation='toggle_inclusion'
            )
            logger.info(f"✅ Indice RAG ottimizzato con successo dopo cambio inclusione file")
        except Exception as e:
            logger.error(f"❌ Errore nell'ottimizzazione dell'indice RAG: {str(e)}")
        return

    # Se il file non è incluso nel RAG, skip indicizzazione
    if not instance.is_included_in_rag:
        logger.info(f"⚠️ File {instance.filename} non incluso nel RAG, skip indicizzazione")
        return

    # Se è una modifica del contenuto, reset flag indicizzazione e ottimizza
    if created or (update_fields and any(field in update_fields for field in ['file_hash', 'file_size', 'file_path'])):
        if not created and instance.is_embedded:
            ProjectFile.objects.filter(id=instance.id).update(is_embedded=False)
            logger.info(f"🔄 Reset flag is_embedded per file {instance.filename}")

        try:
            logger.info(f"🔄 Ottimizzazione indice RAG per file {instance.filename}")
            create_project_rag_chain_optimized(
                instance.project,
                changed_file_id=instance.id,
                operation='content_update'
            )
            ProjectFile.objects.filter(id=instance.id).update(
                is_embedded=True,
                last_indexed_at=timezone.now()
            )
            logger.info(f"✅ File {instance.filename} indicizzato con successo")
        except Exception as e:
            logger.error(f"❌ Errore nell'ottimizzazione dell'indice RAG per file {instance.filename}: {str(e)}")
