...
#                 if langchain_docs:
#                     # MODIFICA 5: Aggiungi i metadati del filename a ogni documento
...
#                 langchain_docs = load_document(doc_model.file_path)
#                 if langchain_docs:
#                     # MODIFICA 5: Aggiungi il titolo del file ai metadati di ogni documento
#                     for doc in langchain_docs:
#                         doc.metadata['filename'] = doc_model.filename
#                         doc.metadata['filename_no_ext'] = os.path.splitext(doc_model.filename)[0]
#
...
#
#         # Controlla se la domanda si riferisce a un nome di file specifico
#         question_lower = question.lower()
#         file_reference = None
#
#         for file in project_files:
#             filename = os.path.basename(file.file_path).lower()
#             filename_no_ext = os.path.splitext(filename)[0].lower()
#
#             # Verifica varie forme di riferimento
#             references = [
#                 f"file {filename}", f"documento {filename}",
#                 f"file {filename_no_ext}", f"documento {filename_no_ext}",
#                 f"allegato {filename}", f"allegato {filename_no_ext}"
#             ]
#
#             for ref in references:
#                 if ref in question_lower or filename in question_lower or filename_no_ext in question_lower:
#                     file_reference = filename
#                     logger.info(f"Rilevato riferimento al file: {filename} nella domanda")
#                     break
#
#             if file_reference:
#                 break
#
#         # Ottieni la catena RAG
...
#         source_documents = result.get('source_documents', [])
#
#         # Se stiamo cercando un file specifico, prioritizza le fonti da quel file
#         if file_reference:
#             # Riordina le fonti mettendo prima quelle del file richiesto
#             source_documents.sort(key=lambda doc:
#             0 if file_reference in doc.metadata.get('filename', '').lower() else 1
#                                   )
#
#         for doc in source_documents:
...
# Gestore centralizzato per l'aggiunta di file
...
#
#     # Aggiorna l'indice vettoriale in background in modo efficiente
#     try:
#         logger.info(f"🔄 Avvio ricostruzione dell'indice vettoriale per il progetto {project.id} dopo caricamento file")
#         # Forza la ricostruzione completa dell'indice con tutti i documenti
#         create_project_rag_chain(project=project, force_rebuild=True)
#         logger.info(f"✅ Indice vettoriale ricostruito con successo per il progetto {project.id}")
#     except Exception as e:
...
#         if was_included:
#             try:
#                 logger.info(f"🔄 Aggiornamento incrementale dell'indice vettoriale dopo eliminazione nota")
#                 update_project_rag_chain(project)
#                 logger.info(f"✅ Indice vettoriale aggiornato con successo")
#             except Exception as e:
#                 logger.error(f"❌ Errore nell'aggiornamento dell'indice: {str(e)}")
